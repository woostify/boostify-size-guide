...
class Metabox {
	...
	public function admin_scripts() {
		...
		if ( $is_size_guide_screen ) {
			wp_register_style( 'boostify-sg-screen-modal', false ); //phpcs:ignore
	...
	public function save_size_guide_metabox( $post_id ) {
		// Checks save status.
		$nonce_name   = ( array_key_exists( 'boostify_sg', $_POST ) ) ? sanitize_text_field( $_POST['boostify_sg'] ) : ''; //phpcs:ignore
		...
		// Display On Category.
		$display = sanitize_text_field( $_POST['bsg_category'] ); //phpcs:ignore
		...
		// Post.
		$product_id = sanitize_text_field( $_POST['bsg_post'] ); //phpcs:ignore
				...
				<div class="condition-group display--on">
					...
					<div class="child-item">
						...
						<select class="display-product-on" multiple="multiple">
							<option value="all">
								<?php echo esc_html__( 'All Product', 'boostify' ); ?>
							</option>
